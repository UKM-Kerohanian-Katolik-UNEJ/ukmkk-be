<x-app-layout>
    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        <form method="post" action="{{ route("admin.proker.update", $proker->slug) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <input type="text" placeholder="Masukkan judul konten" class="input input-bordered w-full @error("judul")
                    input-error
                @enderror" name="judul" value="{{ $proker->judul }}"/>
                @error('judul')
            ...
            <div class="mb-3">
                ...
                <img src="{{ $proker->getFirstMediaUrl("gambar_andalan_konten") }}" id="preview" width="200px">
            ...
            <div class="mb-3">
                ...
                <textarea name="konten">{{ $proker->konten }}</textarea>
                @error('konten')
                    <span class="text-red-700">{{ $message }}</span>
                @enderror
            </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <div class="w-1/2">
                            ...
                            <input type="date" name="tanggal_mulai[]" class="input input-bordered w-full @error("tanggal_mulai")
                                input-error
                            @enderror" value="{{ $jadwal->tanggal_mulai }}" required/>
                            @error('tanggal_mulai')
                                <span class="text-red-700">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="w-1/2">
                            ...
                            <input type="date" name="tanggal_selesai[]" class="input input-bordered w-full @error("tanggal_selesai")
                                input-error
                            @enderror" value="{{ $jadwal->tanggal_selesai }}"/>
