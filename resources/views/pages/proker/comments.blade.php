    ...
    @foreach ($comments as $comment)
        <div class="group px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <div class="flex justify-between gap-3">
                    ...
                    <form action="{{ route("admin.proker.komentar.destroy", $comment->id) }}" method="post">
                        @csrf
                        @method("DELETE")
                        <button
                            class="items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-red-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                            aria-label="Delete"
                        >
                            <svg
                            class="w-5 h-5"
